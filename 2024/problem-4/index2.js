...
fs.readFile(file_path, 'utf8', (err, data) => {
  ...
  const array2D = data
    ...
    for(let i = 0; i <= array2D.length - 1; i++){
        ...
        const safeArray = isSafe(subArray);

        if (safeArray) counter++;
        else {
            if (canBeSafeWithOneRemoval(subArray)) {
                counter++;
            }
        }
    }
...
function canBeSafeWithOneRemoval(arr) {
    for (let i = 0; i < arr.length; i++) {
      const filteredArray = arr.filter((_, index) => index !== i);
      if (isSafe(filteredArray)) {
        return true;
      }
    }
    return false;
